...
public:
    ...
    /*
        shares_selected_k: k selected neighbors in secret-shared form
        label_list_count_array: a 2D array of size k_const × 2
        - first element: occurrence count
        - second element: corresponding label of the neighbor
    */
    ...
    */
    ...
    size: compare_idx_vec must be even. Every two indices form a pair indicating the elements to compare. compare_res stores the comparison results and is duplicated for alignment in multiplication.
    shares: data to be sorted. If shares is a 2D array, sorting is performed based on the first dimension.
    compare_idx_vec: 
    compare_res：
    */
...
class KNN_party_Heap_based:public KNN_party_base
{
public:
    ...
    KNN_party_Heap_based(int playerNo):KNN_party_base(playerNo){
        std::cout<<"Entering the KNN_party_Heap_based class:"<<std::endl;
...
int main(int argc, const char** argv)
{
    ...
    for(int i=0;i<dataset_name_list.size();i++){
        ...
        KNN_party_Heap_based party(playerno);
...
void KNN_party_Heap_based::run()
{
   ...
   for(int i=0;i<num_train_data;++i)
	...
	octetStream os;
	for(int i=k_const;i<num_train_data;++i){
  	   heap[0]={m_ESD_vec[i][0],m_ESD_vec[i][1]};
  	Z2<K> x1 = secure_compare(heap[0][0], heap[1][0], true),tmp=0;
        x1.pack(os);
        m_player->send(os);
        m_player->receive(os);
        tmp.unpack(os);
...
void KNN_party_base::label_compute(vector<Z2<K>>&share_k_neighbors, vector<array<Z2<K>,2>>&label_list_count_array)
{
    ...
    //         Z2<K>tmp_res;
    //         mul_additive(Z2<K>(m_playerno)-U[0],Z2<K>(m_playerno)-U[2],tmp_res);
    //         tmp+=tmp_res;
    //     }
...
// The protocol implemented in this function follows the SecKNN paper
// and is realized within the Garnet framework.
void KNN_party_Heap_based::top_1(vector<array<Z2<K>,2>>&shares,int size_of_need_select,bool min_in_last)
{
...
void KNN_party_base::SS_vec( vector<array<Z2<K>,2>>&shares,const vector<int>compare_idx_vec,const vector<Z2<K>>compare_res)
{
    ...
    h=size_of_cur_cmp>>1;
    for(short i=0;i<h;++i)//tmp_ss stores values collected from shares according to compare_idx_vec, and tmp_res stores the comparison results; both have twice the comparison-result length
    {
...
// The protocol implemented in this function follows the SecKNN paper
// and is realized within the Garnet framework.
void KNN_party_Heap_based::compute_ESD_for_one_query(int idx_of_test)
{
...
void KNN_party_Heap_based::test_additive_share_all_data_function()
{
    Z2<K>tmp_0(0);
    for(int i=0;i<10;i++)
    {
...
void KNN_party_Heap_based::additive_share_all_data()
{
